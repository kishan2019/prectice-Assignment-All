const getFirstItem = (collection, cb) => {
  // invoke the callback function and pass the first item from the collection in as an argumen
  cb(collection[0]);
};
...
const contains = (collection, item, cb) => {
  ...
  const pp = collection.includes(item)? true : false ;
  cb(pp);
};
